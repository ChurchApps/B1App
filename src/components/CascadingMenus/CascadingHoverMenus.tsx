...
const CascadingMenuItem = ({ onClick, ...props }: CascadingMenuItemProps) => {
  ...
  return (<MenuItem {...props} onClick={handleClick} style={{ whiteSpace: "normal", width: 200, overflow: "hidden" }} data-testid={`menu-item-${props.children?.toString().toLowerCase().replace(/\s+/g, "-")}`} aria-label={Locale.label("cascadingMenus.navigateTo").replace("{}", String(props.children))} />);
};
...
const CascadingSubmenu = ({ title, popupId, ...props }: CascadingSubmenuProps) => {
  ...
  return (
    <div>
      <MenuItem {...bindHover(popupState)} {...bindFocus(popupState)} style={{ width: 200 }} data-testid={`submenu-item-${title?.toLowerCase().replace(/\s+/g, "-")}`} aria-label={Locale.label("cascadingMenus.expandSubmenu").replace("{}", title)}>
