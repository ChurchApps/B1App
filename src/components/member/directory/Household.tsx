...
export const Household: React.FC<Props> = (props) => {
  ...
  const getMember = (member: PersonInterface) => {
    ...
    return (<a href="about:blank" className="householdMember" onClick={(e) => { e.preventDefault(); props.selectedHandler(m.id); }} data-testid={`household-member-${m.id}-link`}>
      <Grid container spacing={3}>
        <Grid size={{ xs: 2 }}><img src={PersonHelper.getPhotoUrl(member)} alt={Locale.label("member.directory.avatar")} /></Grid>
        <Grid size={{ xs: 10 }}>
          {member?.name?.display}
    ...
    return (
      <Box sx={{ mt: 2, pt: 2, borderTop: "1px solid #ddd" }}>
        <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 600 }}>
          {Locale.label("member.directory.addFamilyMember")}
        </Typography>
        <Grid container spacing={2} alignItems="center">
          <Grid size={{ xs: 12, sm: 8 }}>
            <TextField
              ...
              helperText={Locale.label("member.directory.addFamilyMemberHelp")}
            ...
            {props.familyMembers.map((name, index) => (
              <Box key={index} sx={{ display: "flex", alignItems: "center", gap: 1, mb: 0.5 }}>
                ...
                <Button size="small" color="error" onClick={() => handleRemoveFamilyMember(index)} data-testid={`household-remove-member-${index}-button`}>
                  {Locale.label("member.directory.remove")}
                </Button>
              </Box>
            ))}
          </Box>
  ...
  return (
    <DisplayBox id="householdBox" headerIcon="people" headerText={Locale.label("member.directory.household")} data-testid="household-display-box">
      {getMembers()}
      {getAddMemberSection()}
    </DisplayBox>
  );
