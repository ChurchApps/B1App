...
export const Wrapper: React.FC<Props> = props => {
  ...
  ConfigHelper.current.tabs.forEach(tab => {
    switch(tab.linkType) {
      case "donation":
        tabs.push(<NavItem key="/donate" url="/donate" label={tab.text} icon={tab.icon} router={router} selected={selectedTab === "donation"} />)
        break;
      case "checkin":
        tabs.push(<NavItem key="/checkin" url="/checkin" label={tab.text} icon={tab.icon} router={router} selected={selectedTab === "checkin"} />)
        break
      case "stream":
        tabs.push(<NavItem key="/stream" url="/stream" label={tab.text} icon={tab.icon} router={router} selected={selectedTab === "stream"} />)
        break
      case "lessons":
        tabs.push(<NavItem key="/lessons" url="/lessons" label={tab.text} icon={tab.icon} router={router} selected={selectedTab === "lessons"} />)
        break
      case "directory":
      ...
      case "page":
        ...
        break
      case "votd":
        tabs.push(<NavItem key="/votd" url="/votd" label={tab.text} icon={tab.icon} router={router} selected={selectedTab === "votd"} />)
        break
      case "groups":
