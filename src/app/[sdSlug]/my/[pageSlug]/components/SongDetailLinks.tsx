...
export const SongDetailLinks = (props: Props) => {
  ...
  const getLink = (link: SongDetailLinkInterface, idx: number) => {
    ...
    let result = <a href={link.url} target="_blank" rel="noreferrer" data-testid={`song-link-${link.service.toLowerCase()}-link`}>{link.service}</a>;
    const logo: string = logos[link.service] as string;
    if (logo) result = <a href={link.url} target="_blank" rel="noreferrer" data-testid={`song-link-${link.service.toLowerCase()}-link`}><img src={logo} alt={link.service} style={{ minHeight: 20, maxHeight: 30, maxWidth: 100 }} /></a>;
