...
export function UnauthenticatedView(props: Props) {
  ...
  const getLeaders = () => {
    ...
    props.leaders.forEach((l) => {
      result.push(<Grid container size={{ xs: 6, md: 2 }} key={l.person.id} style={{ height: "50px", backgroundColor: "hsl(0, 0%, 85%)", marginLeft: "5px", borderRadius: "40px" }}>
        <Grid size={{ md: 4 }} style={{ padding: "5px" }}>
        ...
        <Grid size={{ md: 8 }} style={{ lineHeight: "50px" }}><a href={"/mobile/community/" + l.person?.id} style={{ color: "black" }} data-testid={`group-leader-${l.person?.id}-link`}>{l.person?.name?.display}</a></Grid>
      </Grid>);
    });
    return result;
  };
    ...
    props.events.forEach((e) => {
      ...
      if (result.length < 3 && startDate > currDate) {
        result.push(<Grid container key={e.id}>
          <Grid container size={{ xs: 4, md: 4 }} style={{ height: "80px", marginBottom: "10px" }}>
            <Grid size={{ xs: 5, md: 5 }}></Grid>
            <Grid size={{ xs: 7, md: 7 }} className="calbox" style={{ lineHeight: "80px", borderRadius: "15%", fontWeight: "bold" }}>
              <p style={{ lineHeight: "16px" }}>{monthAbb}</p>
              <p style={{ lineHeight: "16px" }}>{dayShort}</p>
            </Grid>
          </Grid>
          <Grid size={{ xs: 8, md: 8 }} style={{ textAlign: "left", lineHeight: "40px", paddingLeft: "5px" }}>
  ...
  return <>
    ...
    <Container>
      ...
      <Grid container spacing={4}>
        <Grid size={{ md: shift, xs: 12 }} style={{ textAlign: "center" }}>
