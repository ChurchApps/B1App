  ...
  const getLeaders = () => {
    ...
    props.leaders.forEach((l) => {
      result.push(<Grid container size={{ xs: 6, md: 2 }} key={l.person.id} style={{ height: "50px", backgroundColor: "hsl(0, 0%, 85%)", marginLeft: "5px", borderRadius: "40px" }}>
        ...
        <Grid size={{ md: 8 }} style={{ lineHeight: "50px" }}><a href={"/my/community/" + l.person?.id} style={{ color: "black" }} data-testid={`group-leader-${l.person?.id}-link`}>{l.person?.name?.display}</a></Grid>
      </Grid>);
    });
    return result;
  };
