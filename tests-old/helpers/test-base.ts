...
export class TestHelpers {
  ...
  /**
   * Comprehensive cache and state clearing function
   * Ensures user is completely logged out and browser state is clean
   */
  static async clearAllCacheAndCookies(page: Page) {
    console.log('=== Clearing all browser cache, cookies, and state ===');
    ...
    try {
      ...
      await page.evaluate(() => {
        ...
        if (typeof localStorage !== 'undefined') {
          ...
          localStorage.clear();
          console.log(`Cleared localStorage (${localStorageLength} items)`);
        }
      ...
      console.log('⚠️ Could not clear all storage:', error);
    }
    
    // Wait for everything to clear before proceeding
    await page.waitForTimeout(2000);
    
    // Verify we're not authenticated by checking if we can access a public page
    await page.goto('/', { waitUntil: 'domcontentloaded' });
    console.log('✅ Browser state completely cleared and verified');
  }
